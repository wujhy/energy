...
6. `ControlBattery` 内的 `cmdBatteryControlService` 和 `CommServer.returnCmd` 仍是旧 M460/980 命令链路入口，只适合历史兼容和逐步清理，不应继续作为新测试计划的默认运行路径。
7. `_1` 内阻测试在旧立即执行链路有模式互斥和 5 分钟重复限制，但当前独立采集命令服务没有明确“整组内阻测试”高级入口，不能简单等同 `_6` 单节内阻测试。
8. `_3` 核容、`_5` 备电时长属于长周期放电/后处理闭环，当前只能暂缓，不应在采集线程或普通命令队列里临时闭环。
9. `_4` 浮充测试、`_7` 充电测试缺少已核验的 M460 命令和运行态闭环，暂不新增映射。
...
#### TASK-CODEX-M460-OPTPLAN-003：清理旧测试指令直发链路

目标：把 `ControlBattery` 里仍依赖 `cmdBatteryControlService` + `CommServer.returnCmd` 的旧 M460 测试指令路径明确标为待清理/待迁移，避免新测试计划继续走旧链路。

对象：

1. `ControlBattery`
2. `CommServer`
3. `CmdBatteryControlService`
4. `BatterySyncHandler`

输出：

1. 哪些方法仍属于旧 M460/980 直发链路。
2. 哪些方法已经有 600 队列替代入口，哪些还没有。
3. 新测试计划不应再调用的旧入口清单。
4. 最小清理任务拆分。

约束：

1. 只做链路识别、注释、计划标记和迁移边界说明，不在本任务里重写业务流程。
2. 不改 `rysqlite3.db`。
3. 不把旧兼容路径和新 600 队列实现混到一个任务里。
4. 需要保留旧行为的地方，必须明确说明“仅兼容历史流程”。
...
### TASK-CODEX-M460-STOP-001：600 测试停止和队列取消闭环

目标：实现测试停止的 600 队列语义，替代 `_2/_6` 使用旧 `genCmd30(..., "4")` 或直接拒绝停止的临时做法。

已确认规则：

1. 停止请求只针对当前电池组。
2. 必须先判断当前组正在执行的测试类型是否等于请求停止的 `testType`。
3. 类型不匹配时不停止其他测试，不清理无关队列。
4. 类型匹配时，停止队列中尚未下发的相关测试指令。
5. 已经下发到设备的帧不能伪造撤回，只能按当前命令状态和响应结果收尾。
6. 停止后同步更新运行状态、`dev_opt_log` 和 600 命令日志。

对象：

1. `ControlBattery.toSendStopBatteryCmdToOat`
2. `BatteryOptCollectorCommandAdapter`
3. `BatteryCollectorCommandService`
4. `BatteryCollectorCommandQueueService`
5. `BatteryModeStatusService`
6. `OptLogService`
7. `BatteryCollectorCommandLogService`

实现边界：

1. `_2` 连接条测试和 `_6` 单节内阻测试优先接入 600 队列取消。
2. `_3/_5` 核容/备电停止暂时保留旧兼容路径，后续等容量/备电闭环任务处理。
3. 不恢复 `cmdBatteryControlService.genCmd30(..., "4")` 作为 `_2/_6` 停止方案。
4. 不把计划保存、立即执行、停止取消三个流程混在一个提交里。

验收：

1. 当前组无运行测试时，停止请求返回明确失败或幂等成功，不能清理其他组队列。
2. 当前组运行类型与停止类型不一致时，不清理队列、不关闭日志。
3. 当前组运行类型匹配时，未下发命令被取消，运行状态和日志闭环。
4. `rg "genCmd30\(.*\"4\"" ControlBattery.java` 不能重新覆盖 `_2/_6`。
