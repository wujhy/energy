...
- `_3` 核容测试已收进 `BatteryOptCapacityModuleCommandAdapter`，但仅作为占位备注指向 M460 底层 `0x30 mode=2/4` 对应功能，不做兼容下发，不和 `_5` 外部备电直控混为同一套下层设备能力。
...
- `BatteryOptCapacityModuleCommandAdapter`
  - 只承接 `_5` 备电时长外部模块控制
  - `_3` 核容只在 adapter 内占位备注 M460 底层 `0x30 mode=2/4` 对应功能，不做兼容下发
  - 不处理通用空开业务
...
- `ControlBattery`
  ...
  - `ControlBattery` 旧 `generateCommand/executeCommandAndLog` fallback 已清理，`_3` 占位收进 adapter 边界，不保留兼容下发代码
...
`_3` 已收进 `BatteryOptCapacityModuleCommandAdapter` 作为占位分流：注释和日志只指向 M460 底层 `0x30 mode=2/4` 核容/空闲对应功能，不保留 `genCmd30/_E0` 兼容下发代码。

当前 `_3` 进入 adapter 后返回 `null`，由 `ControlBattery` 统一返回“禁止回退旧 M460 指令”的明确错误；不创建 running log，不新增执行、停止、状态、结果接入或调度工作。
