...
class ControlBatteryTest {
    ...
    void shouldCheckConnectResistanceCurrentBeforeCollectorCommandAdapter() {
        ...
        BatteryOptCollectorCommandAdapter commandAdapter = field(service, "batteryOptCollectorCommandAdapter");
        Mockito.when(snapshotService.getCachedSnapshot(1)).thenReturn(snapshot(BatteryPackStatusEnum.MONITOR.getCode(), 1D));
    ...
    void shouldRejectWhenActiveAlarmExists() {
        ...
        BatteryOptCollectorCommandAdapter commandAdapter = field(service, "batteryOptCollectorCommandAdapter");
        Mockito.when(snapshotService.getCachedSnapshot(1)).thenReturn(snapshot(BatteryPackStatusEnum.MONITOR.getCode(), 10D));
    ...
    @Test
    void shouldRejectCommandWhenModeStatusRunning() {
        ControlBattery service = service();
        BatteryModeStatusService modeStatusService = field(service, "batteryModeStatusService");
        ...
        modeInfo.setStatus(BatteryModeStatusService.STATUS_RUNNING);
    ...
    private ControlBattery service() {
        ...
        ReflectionTestUtils.setField(service, "batteryModeStatusService", Mockito.mock(BatteryModeStatusService.class));
    ...
    private BatteryModuleRealtimeSnapshot snapshot(String status, Double current) {
        ...
        group.setChargeDischargeCurrent(current);
